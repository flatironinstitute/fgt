...
c    $Revision$
      ...
      subroutine bfgt2dpght(nd,delta,eps,iperiod,nboxes,nlevels,ltree,
...
c
      ...
      real *8 grad(nd,2,npbox,nboxes)
      real *8 hess(nd,3,npbox,nboxes)
      real *8 pote(nd,ntarg)
      real *8 grade(nd,2,ntarg)
      real *8 hesse(nd,3,ntarg)
...
c
      call cpu_time(time1)
C$      time1=omp_get_wtime()
      call bfgt2dpghtmain(nd,delta,eps,nboxes,nlevels,ltree,
     1    itree,iptr,norder,ncbox,ttype,fvals,centers,boxsize,
     2    iaddr,rmlexp,npwlevel,pmax,npw,npbox,
...
c
c
      subroutine bfgt2dpghtmain(nd,delta,eps,nboxes,nlevels,ltree,
     1    itree,iptr,norder,ncbox,ttype,fvals,centers,boxsize,
     2    iaddr,rmlexp,npwlevel,pmax,npw,npbox,
     3    ifpgh,pot,grad,hess,
     4    ntarg,targs,ifpghtarg,pote,grade,hesse,timeinfo)
...
c     compute the tables converting planewave expansions to potential values
      ...
      do ilev=nlevstart,nlevels
ccc         call mk_pw2pot(norder,npw,ts,xq,delta,boxsize(ilev),
ccc     1       tab_pw2pot(1,1,ilev))      
         call mk_pw2pgh(norder,npw,ts,xq,delta,boxsize(ilev),
...
cccc                nb=nb+1
                call cpu_time(t1)
ccc                call g2d_pw2pot(nd,norder,npw,rmlexp(iaddr(2,ibox)),
ccc     1              gg,tab_pw2pot(1,1,ilev),pot(1,1,ibox))
                if (ifpgh.eq.1) then
                   call g2d_pw2pot(nd,norder,npw,
     1              rmlexp(iaddr(2,ibox)),gg,
     2              tab_pw2pot(1,1,ilev),
     3              pot(1,1,ibox))
                endif
                if (ifpgh.eq.2) then
                   call g2d_pw2pg(nd,norder,npw,
     1              rmlexp(iaddr(2,ibox)),gg,gg2,
     2              tab_pw2pot(1,1,ilev),tab_pw2potx(1,1,ilev),
     3              pot(1,1,ibox),grad(1,1,1,ibox))
                endif
                if (ifpgh.eq.3) then
                   call g2d_pw2pgh(nd,norder,npw,
     1              rmlexp(iaddr(2,ibox)),gg,gg2,gg3,
     2              tab_pw2pot(1,1,ilev),tab_pw2potx(1,1,ilev),
     2              tab_pw2potxx(1,1,ilev),
     3              pot(1,1,ibox),grad(1,1,1,ibox),hess(1,1,1,ibox))
                endif
ccc                 call prin2(' after pw2pgh pot *',pot(1,1,ibox),1)
ccc                 call prin2(' after pw2pgh grad *',grad(1,1,1,ibox),1)
...
C$OMP END PARALLEL DO         
 2000 continue
c
      call mklegtables(norder,xq,tabf,tabfx,tabfxx)
ccc      call prin2(' tabf =*',tabf,norder*norder)
ccc      call prin2(' tabfx =*',tabfx,norder*norder)
ccc      call prin2(' tabfxx =*',tabfxx,norder*norder)
ccc      goto 3001
...
c        ibox is the source box            
            if (ifhung(ibox) .eq. 1) then
ccc               call prin2(' pot =*',pot(1,1,ibox),norder*norder)
               call legtrans2d(nd,norder,pot(1,1,ibox),lgcoefs,umat)
ccc               call prin2(' lgcoefs =*',lgcoefs,norder*norder)
               if (ifpgh.eq.2) then
