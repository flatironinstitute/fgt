...
ccc         enddo
         ...
         do j=1,(npw+1)/2
            tab_pw2pot(j,i)=exp(eye*ts(j)*x)
            tab_pw2potx(j,i)= eye*ts(j)*dsq*tab_pw2pot(j,i)*2/boxdim
            tab_pw2potxx(j,i)= eye*ts(j)*dsq*tab_pw2potx(j,i)*2/boxdim
            tab_pw2pot(npw-j+1,i)=dconjg(tab_pw2pot(j,i))
            tab_pw2potx(npw-j+1,i)=dconjg(tab_pw2potx(j,i))
            tab_pw2potxx(npw-j+1,i)=dconjg(tab_pw2potxx(j,i))
         enddo
...
c
C*********************************************************************C
      subroutine mk_loctab_collpgh(n,nnodes,delta,boxdim,
     1           tab_colleague,tab_colleaguex,tab_colleaguexx)
C*********************************************************************C
c     This routine is a correct but not optimized table generator.
c
...
c----------------------------------------------------------------------c
      ...
      real *8 tab_colleaguexx(n,n,-1:1)
      real *8, allocatable :: whts(:), xnodes(:)
...
c
      do m = 1,n
         do j = 1,n
            ...
            do i = 1,nnodes
               dx = xi - xnodes(i)
               rsum = rsum + (-2.0d0/delta + 4.0d0*dx*dx/(delta**2))*
     1             legev(m,i)*exp(-dx*dx/delta)*whts(i)
               
               dx = xim1 - xnodes(i)
               rsumm1=rsumm1+(-2.0d0/delta + 4.0d0*dx*dx/(delta**2))*
     1             legev(m,i)*exp(-dx*dx/delta)*whts(i)
               
               dx = xip1 - xnodes(i)
               rsump1=rsump1+(-2.0d0/delta + 4.0d0*dx*dx/(delta**2))*
     1             legev(m,i)*exp(-dx*dx/delta)*whts(i)
            enddo
            tab_colleaguexx(m,j,-1) = rsumm1
            tab_colleaguexx(m,j,0)  = rsum
            tab_colleaguexx(m,j,1)  = rsump1
         enddo
      enddo
...
c
C*********************************************************************C
      subroutine mk_loctab_stobpgh(n,nnodes,delta,boxdim,
     1           tab_stob,tab_stobx,tab_stobxx)
C*********************************************************************C
c     This routine is a correct but not optimized table generator.
c
c     tab_colleague(n,j,k) = 
...
c
      do m = 1,n
         do j = 1,n
            ...
            do i = 1,nnodes
               dx = xi1 - xnodes(i)
               rsum1 = rsum1 + (-2.0d0/delta + 4.0d0*dx*dx/(delta**2))*
     1             legev(m,i)*exp(-dx*dx/delta)*whts(i)
               
               dx = xi2 - xnodes(i)
               rsum2 = rsum2 + (-2.0d0/delta + 4.0d0*dx*dx/(delta**2))*
     1             legev(m,i)*exp(-dx*dx/delta)*whts(i)

               dx = xi3 - xnodes(i)
               rsum3 = rsum3 + (-2.0d0/delta + 4.0d0*dx*dx/(delta**2))*
     1             legev(m,i)*exp(-dx*dx/delta)*whts(i)
               
               dx = xi4 - xnodes(i)
               rsum4 = rsum4 + (-2.0d0/delta + 4.0d0*dx*dx/(delta**2))*
     1             legev(m,i)*exp(-dx*dx/delta)*whts(i)
            enddo
            
            tab_stobxx(m,j,1) = rsum1
            tab_stobxx(m,j,2) = rsum2
            tab_stobxx(m,j,3) = rsum3
            tab_stobxx(m,j,4) = rsum4
         enddo
      enddo
...
c
C*********************************************************************C
      subroutine mk_loctab_btospgh(n,nnodes,delta,boxdim,
     1           tab_btos,tab_btosx,tab_btosxx)
C*********************************************************************C
c     This routine is a correct but not optimized table generator.
c
c     tab_colleague(n,j,k) = 
...
c
     ...
     1         legev(m,i)*exp(-dx*dx/delta)*whts(i)
            enddo
            tab_btosxx(m,j,1) = rsum1
            tab_btosxx(m,j,2) = rsum2
            tab_btosxx(m,j,3) = rsum3
            tab_btosxx(m,j,4) = rsum4
         enddo
      enddo
