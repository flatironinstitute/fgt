...
c
      ...
      implicit none
      integer nlevels,levcut,nboxes,idivflag
      integer *8 ltree,nboxes8
      integer nbmax,nbtot
      integer ns,nt,ndiv,ndiv0
      integer nlmin,iper,ifunif
      double precision src(ns),targ(nt),cen0,bs0
     ...
     1    ichild2(:,:),isrcse2(:,:),itargse2(:,:)
      ...
      allocate(centers(nbmax),isrcse(2,nbmax),itargse(2,nbmax))
...
c
        if(nbtot.gt.nbmax) then
          ...
          allocate(nchild2(nbmax),ichild2(2,nbmax),isrcse2(2,nbmax))
...
C$OMP END PARALLEL DO          
     ...
     1        isrcse,itargse)
          ...
          allocate(nchild(nbmax),ichild(2,nbmax),isrcse(2,nbmax))
...
c
      ...
      implicit none
      integer nlevels,nboxes,ns,nt,idivflag,ndiv,levcut,ndiv0
      integer *8 iptr(8),ltree
      integer itree(ltree),iper
      integer ifunif,nlmin,nlmax
      double precision centers(nboxes),src(ns),targ(nt)
      real *8 bs0,cen0
      integer, allocatable :: irefinebox(:)
...
c
      do iss = isrcse(1,ibox),isrcse(2,ibox)
        if(src(isrc(iss))-centers(ibox).lt.0) then
...
c      
      ...
      real *8 src(ns),targ(nt),bs0,cen0
      
      xmin = src(1)
      xmax = src(1)

      do i=1,ns
        if(src(i).lt.xmin) xmin = src(i)
        if(src(i).gt.xmax) xmax = src(i)
      enddo

      do i=1,nt
        if(targ(i).lt.xmin) xmin = targ(i)
        if(targ(i).gt.xmax) xmax = targ(i)
      enddo
