...
C$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(i,ibox,nbl,j,jbox)
      do i = 1,nbloc
        ...
        if(irefinebox(i).eq.1) then
          ...
          do j=1,2
            jbox = nbl+j
            centers(jbox) = centers(ibox)+(-1)**j*bs/2
            iparent(jbox) = ibox
            nchild(jbox) = 0
            do l=1,2
              ichild(l,jbox) = -1
...
c---------------------------------------------------------------
      ...
      integer iparent(nboxes), nchild(nboxes), ichild(2,nboxes)
      integer nnbors(nboxes)
      integer nbors(3,nboxes)
...
c
      ...
      real *8 centers(nboxes),boxsize(0:nlevels)
      character (len=*) fname

      open(unit=33,file=trim(fname))
      

 1111 format(2(2x,e11.5))      

      do ibox=1,nboxes
         if(itree(iptr(4)+ibox-1).eq.0) then
           ilev = itree(iptr(2)+ibox-1)
           bs = boxsize(ilev)
           x1 = centers(ibox) - bs/2
           x2 = centers(ibox) + bs/2
...
c              Loop over all colleagues of ibox              
               do i=1,itree(iptr(6)+ibox-1)
                  jbox = itree(iptr(7)+3*(ibox-1)+i-1)
...
c                       Loop over children of colleague box              
                        do j=1,itree(iptr(4)+jbox-1)
                           kbox = itree(iptr(5)+2*(jbox-1)+j-1)
                           xdis = dabs(centers(kbox)-centers(ibox))
c                       Test to see if child of colleague box
c                       is in list1
                           if(xdis.lt.distest) then
                              nlist1(ibox) = nlist1(ibox)+1
                              list1(nlist1(ibox),ibox)=kbox
...
c
c
      subroutine gt1d_computemnlists(nlevels,nboxes,itree,ltree,
     1   iptr,centers,
     1   boxsize,iper,mnlistsoe)
c
...
c
c
      subroutine gt1d_computelists_slow(nlevels,npwlevel,nboxes,
     1    itree,ltree,iptr,centers,boxsize,laddr,
     2    mnlistsoe,nlistsoe,listsoe)
c     this subroutine returns lists of various types of SOE and SOE/X boxes
c
c     input parameters:
c
...
c
c     mnlistsoe = maximum number of a particular type of SOE expansions
c     mnlistsx = maximum number of a particular type of SOE/X expansions
c      
c     output parameters:
c
c     nlistsoe (0:1,nboxes) = contains the number of boxes for each particular
c                             type of SOE expansions for each box
c
c     listsoe (mnlistsoe,0:3,nboxes) = contains the box ID for each particular
c                             type of SOE expansions for each box
c
c
C     SOE expansion type index: 0-p, 1-m
c      
c      
      implicit real *8 (a-h,o-z)
      integer nlevels,nboxes,ltree
      ...
      real *8 centers(nboxes),boxsize(0:nlevels)
      integer mnlistsoe,mnlistsx
      integer nlistsoe(0:1,nboxes), listsoe(mnlistsoe,0:1,nboxes)
      ...
      do ilev=nlevstart,nlevels
        ...
        do ibox = laddr(1,ilev),laddr(2,ilev)
          if (ilev .eq. npwlevel) then
            ...
            do i=1,ncoll
              jbox = itree(iptr(7) + (ibox-1)*3+i-1)
              if (ibox .ne. jbox) then 
              ix = 1.05d0*(centers(jbox)-centers(ibox))/bs
              if(ix.lt.0) then
          ...
          elseif (ilev .gt. max(npwlevel,0)) then
            ...
            do i=1,nchild
               jbox = itree(iptr(5)+2*(idad0-1)+i-1)
               ix = 1.05d0*(centers(jbox)-centers(ibox))/bs
...
c         end of ilev if statement
          endif
cccc          write(13,*) ilev, nlistsx(0,ibox),nlistsx(1,ibox),
cccc     1        nlistsx(2,ibox),nlistsx(3,ibox)
c       end of ibox do loop
        enddo
