...
c
      do i=1,13
         deltas(i)=1d-5*2**(13-i)*2
      enddo

      do i=1,13
         nsrcs(i)=10**7/2**(13-i)*2
      enddo
...
c
      ...
      if (ifuniform.eq.1) then
         open(iw, file='errorN2d.txt', position='append')
         open(iw2, file='timingN2d.txt', position='append')
      else
         open(iw, file='errorN2d.txt', position='append')
         open(iw2, file='timingN2d.txt', position='append')
      endif
...
c     
      ...
      do j=1,13
         write(iw2,*) nsrcs(j), pps(j,1), pps(j,2), pps(j,3)
      enddo
