...
c    $Revision$
      ...
      subroutine bfgt1d(nd,delta,eps,nboxes,nlevels,ltree,
...
c
      call cpu_time(time1)
C$      time1=omp_get_wtime()
      call bfgt1dmain(nd,delta,eps,nboxes,nlevels,ltree,
...
c
c
      subroutine bfgt1dmain(nd,delta,eps,nboxes,nlevels,ltree,
     1    itree,iptr,norder,ncbox,ttype,fvals,centers,boxsize,
     2    iaddr,rmlexp,npwlevel,pmax,npw,
     3    npbox,pot,timeinfo)
...
c
      ...
      real *8 fvals(nd,npbox,nboxes)
      real *8 pot(nd,npbox,nboxes)
      real *8 boxsize(0:nlevels),centers(nboxes)
     ...
     1   vmat(norder,norder),wts(norder))
      ...
      do ilev = 0,nlevels
        do ibox = itree(2*ilev+1),itree(2*ilev+2)
          ...
          if(nchild.eq.0)
     1        call legtrans1d(nd,norder,fvals(1,1,ibox),
         ...
         do ilev=npwlevel,npwlevel
            do ibox=itree(2*ilev+1),itree(2*ilev+2)
               ...
               if (nchild .eq. 0) then
                  ...
                  do j=1,ncoll
                     jbox = itree(iptr(7) + (ibox-1)*3+j-1)
...
c     compute the tables converting planewave expansions to potential values
     ...
     1    wpwshift)
      ...
      allocate(wpwmsshift(nexp,2,nmax))
...
c     compute list info
c
      call gt1d_computemnlistpw(nlevels,nboxes,itree,ltree,
     1    iptr,centers,
...
c                 form PW expansion directly
                  call leg1d_to_pw(nd,norder,fcoefs(1,1,ibox),npw,
     1                tab_leg2pw(1,1,ilev),rmlexp(iaddr(1,ibox)))
                  call cpu_time(t2)
                  dt=dt+t2-t1
               endif
            endif
...
C$OMP END PARALLEL DO
 111     format ('ilev=', i2,4x, 'nb=',i6, 4x,'formpw=', f5.2)         
...
C$OMP$SCHEDULE(DYNAMIC)
        do ibox = itree(2*ilev+1),itree(2*ilev+2)
          ...
          do i=1,nchild
             ...
             call g1dshiftpw_vec(nd,nexp,rmlexp(iaddr(2,ibox)),
...
c            do nothing here
             ...
             if(nchild.eq.0) then
                nb=nb+1
                call cpu_time(t1)
                call g1d_pw2pot(nd,norder,npw,rmlexp(iaddr(2,ibox)),
     1              tab_pw2pot(1,1,ilev),pot(1,1,ibox))
                call cpu_time(t2)
                dt=dt+t2-t1
             endif
           endif
ccc    end of ibox loop        
        enddo
 222    format ('ilev=', i2,4x, 'nb=',i6, 4x,'pweval=', f5.2)         
...
c                 colleague                  
                  if (ilev .eq. jlev) then
                     ...
                     ix = (centers(jbox)-centers(ibox))/bs

                     call leg1d_to_potloc(nd,norder,fcoefs(1,1,ibox),
     1                   pot(1,1,jbox),
     2                   tab_coll(1,1,ix,jlev))
                     endif
c                 big source box to small target box                     
                  elseif (ilev .eq. jlev-1) then
                     ix = (centers(jbox)-centers(ibox))/bs+2.55d0
...
c                 small source box to big target box 
                  elseif (ilev .eq. jlev+1) then
                     ix = (centers(jbox)-centers(ibox))/bs*2+2.55d0
