...
cc          compute list1 at level ilev-1 
            do i=1,nnbors(dad)
               ...
               if(nchild(jbox).eq.0) then
                  distest = 1.05d0*(boxsize(ilev)+boxsize(ilev-1))/
     1                2.0d0*isep
                  xdis = dabs(centers(jbox)-centers(ibox))
                  if(xdis.lt.distest) then
                     nlist1(ibox) = nlist1(ibox)+1
                     list1(nlist1(ibox),ibox) = jbox
                  endif
...
c      
c      
      subroutine gt1d_computemnlistpw(nlevels,nboxes,itree,ltree,
     1   iptr,centers,
...
c
c
      subroutine gt1d_computelistpw(nlevels,npwlevel,nboxes,
...
c      
      ...
      real *8 centers(nboxes),boxsize(0:nlevels)
      integer nchild(nboxes),ichild(2,nboxes)
      integer mnlistpw
