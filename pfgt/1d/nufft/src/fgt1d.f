...
c
      ...
      real *8 time1,time2,pi,done,pmax,bs0,cen0,bsize,pweps
...
c
      ...
      if (npwlevel .lt. -2) then
         call fgthlterms(ndim,bs0,delta,eps,nlocal0)
...
c     than 20 for high precision calculation
      ...
      do i = nlevstart,100
         call fgthlterms(ndim,bsize,delta,eps,nlocal0)
...
c
      ...
      do i = max(npwlevel,0),nlevels
         call fgthlterms(ndim,boxsize(i),delta,eps,nlocal(i))
...
c     compute the length of plane wave expansion
      ...
      call fgtpwterms(bsize,delta,pweps,pmax,npw)
...
c     reorder sources
c
      call dreorderf(1,ns,sources,sourcesort,isrc)
      if(ifcharge.eq.1) 
     1    call dreorderf(nd,ns,charge,chargesort,isrc)
      if(ifdipole.eq.1) then
         call dreorderf(nd,ns,dipstr,dipstrsort,isrc)
         call dreorderf(1,ns,rnormal,rnormalsort,isrc)
      endif
c
cc     reorder targets
c
      call dreorderf(1,nt,targ,targsort,itarg)
c
c
...
c
      ...
      integer nn,jx,nlevstart,nlevend
...
c     compute translation matrices for PW expansions
      ...
      nmax = 1
      nexp = npw/2
      allocate(wpwshift(nexp,-nmax:nmax))
...
c     listpw contains source boxes in the pw interaction
      call gt1d_computelistpw(nlevels,npwlevel,nboxes,
...
cccc           call g1dsoezero_vec(nd,rmlexp(iaddr(1,ibox)),npw)
cccc           call g1dsxzero_vec(nd,rmlexp(iaddr(2,ibox)),
cccc     1          npw,npwhalf)
cccc           call g1dsoezero_vec(nd,rmlexp(iaddr(3,ibox)),npw)
...
c                 for self interaction 
                  call g1dcopypwexp_vec(nd,nexp,rmlexp(iaddr(1,ibox)),
...
c              Check if current box needs to form pw exp          
               if(npts.gt.ndiv) then
                  if (npts.gt.ncddir) then
                     call g1dformpwcd_fast_vec(nd,delta,eps,
     1                   sourcesort(istart),npts,chargesort(1,istart),
     2                   rnormalsort(istart),dipstrsort(1,istart),
     3                   centers(ibox),
     4                   npw,ws,ts,wnufft,rmlexp(iaddr(1,ibox)))
                  else
                     call g1dformpwcd_vec(nd,delta,
     1                   sourcesort(istart),npts,chargesort(1,istart),
     2                   rnormalsort(istart),dipstrsort(1,istart),
     3                   centers(ibox),
     4                   npw,ws,ts,rmlexp(iaddr(1,ibox)))
...
C     $OMP END PARALLEL DO
 ...
 111     format ('ilev=', i2,4x, 'nb=',i6, 4x,'formpw=', f6.2)
...
c
            do j=1,nlistpw(ibox)
              ...
              if (nptsj .gt. ndiv) then
                jx= nint((centers(ibox) - centers(jbox))/xmin)

                call g1dshiftpw_vec(nd,nexp,rmlexp(iaddr(1,jbox)),
     1              rmlexp(iaddr(2,ibox)),wpwshift(1,jx))
              endif
            enddo
          endif
        enddo
...
c     evaluate local expansion at targets
               if(nptstarg.gt.0) then
                  if (ifpghtarg.eq.1.and.nptstarg.gt.npdir) then
                     call g1dpwevalp_fast_vec(nd,delta,eps,
     1                   centers(ibox),npw,ws,ts,
     2                   rmlexp(iaddr(2,ibox)),
     3                   targetsort(istartt),
     4                   nptstarg,pottarg(1,istartt))
                  elseif (ifpghtarg.eq.1.and.nptstarg.le.npdir) then
                     call g1dpwevalp_vec(nd,delta,
     1                   centers(ibox),npw,ws,ts,
     2                   rmlexp(iaddr(2,ibox)),
     3                   targetsort(istartt),
     4                   nptstarg,pottarg(1,istartt))
                  ...
                  if (ifpghtarg.eq.2.and.nptstarg.gt.ngdir) then
                     call g1dpwevalg_fast_vec(nd,delta,eps,
     1                   centers(ibox),npw,ws,ts,
     2                   rmlexp(iaddr(2,ibox)),
     3                   targetsort(istartt),
     4                   nptstarg,pottarg(1,istartt),
     5                   gradtarg(1,istartt))
                  elseif (ifpghtarg.eq.2.and.nptstarg.le.ngdir) then
                     call g1dpwevalg_vec(nd,delta,
     1                   centers(ibox),npw,ws,ts,
     2                   rmlexp(iaddr(2,ibox)),
     3                   targetsort(istartt),
     4                   nptstarg,pottarg(1,istartt),
     5                   gradtarg(1,istartt))
                  ...
                  if (ifpghtarg.eq.3.and.nptstarg.gt.nhdir) then
                     call g1dpwevalh_fast_vec(nd,delta,eps,
     1                   centers(ibox),npw,ws,ts,
     2                   rmlexp(iaddr(2,ibox)),
     3                   targetsort(istartt),
     4                   nptstarg,pottarg(1,istartt),
     5                   gradtarg(1,istartt),
     6                   hesstarg(1,istartt))
                  elseif (ifpghtarg.eq.3.and.nptstarg.le.nhdir) then
                     call g1dpwevalh_vec(nd,delta,
     1                   centers(ibox),npw,ws,ts,
     2                   rmlexp(iaddr(2,ibox)),
     3                   targetsort(istartt),
     4                   nptstarg,pottarg(1,istartt),
...
c     evaluate local expansion at sources
     ...
     2                   rmlexp(iaddr(2,ibox)),
     3                   sourcesort(istarts),nptssrc,
     4                   pot(1,istarts),grad(1,istarts))
     ...
     5                   hess(1,istarts))
         ...
         enddo
         call cpu_time(t2)
 222     format ('ilev=', i2,4x, 'nb=',i6, 4x,'pweval=', f6.2)
...
cccc              ibox is the target box
                  ...
                  if (nptstarg .gt. 0) then
                     call fgt1dpart_direct_vec(nd,delta,dmax,
...
c
c------------------------------------------------------------------     
      subroutine fgt1dpart_direct_vec(nd,delta,dmax,istart,iend,
...
c
        ...
        if(ifcharge.eq.1.and.ifdipole.eq.0) then
          if(ifpgh.eq.1) then
             call g1d_directcp_vec(nd,delta,dmax,source(istart),ns,
     1           charge(1,istart),targ(jstart),ntarg,pot(1,jstart))
          endif

          if(ifpgh.eq.2) then
             call g1d_directcg_vec(nd,delta,dmax,source(istart),ns,
     1           charge(1,istart),targ(jstart),ntarg,pot(1,jstart),
     2           grad(1,jstart))
          endif
          if(ifpgh.eq.3) then 
             call g1d_directch_vec(nd,delta,dmax,source(istart),ns,
     1           charge(1,istart),targ(jstart),ntarg,pot(1,jstart),
     2           grad(1,jstart),hess(1,jstart))
          endif
        endif

        if(ifcharge.eq.0.and.ifdipole.eq.1) then
          if(ifpgh.eq.1) then
             call g1d_directdp_vec(nd,delta,dmax,source(istart),ns,
     1           rnormal(istart),dipstr(1,istart),targ(jstart),
     2           ntarg,pot(1,jstart))
          endif

          if(ifpgh.eq.2) then
             call g1d_directdg_vec(nd,delta,dmax,source(istart),ns,
     1           rnormal(istart),dipstr(1,istart),targ(jstart),
     2           ntarg,pot(1,jstart),grad(1,jstart))
          endif
          if(ifpgh.eq.3) then
             call g1d_directdh_vec(nd,delta,dmax,source(istart),ns,
     1           rnormal(istart),dipstr(1,istart),targ(jstart),
     2           ntarg,pot(1,jstart),grad(1,jstart),hess(1,jstart))
          endif
        endif 

        if(ifcharge.eq.1.and.ifdipole.eq.1) then
          if(ifpgh.eq.1) then
             call g1d_directcdp_vec(nd,delta,dmax,source(istart),ns,
     1           charge(1,istart),rnormal(istart),dipstr(1,istart),
     2           targ(jstart),ntarg,pot(1,jstart))
          endif

          if(ifpgh.eq.2) then
             call g1d_directcdg_vec(nd,delta,dmax,source(istart),ns,
     1           charge(1,istart),rnormal(istart),dipstr(1,istart),
     2           targ(jstart),ntarg,pot(1,jstart),grad(1,jstart))
          endif
          if(ifpgh.eq.3) then
             call g1d_directcdh_vec(nd,delta,dmax,source(istart),ns,
     1           charge(1,istart),rnormal(istart),dipstr(1,istart),
     2           targ(jstart),ntarg,pot(1,jstart),grad(1,jstart),
     3           hess(1,jstart))
          endif
