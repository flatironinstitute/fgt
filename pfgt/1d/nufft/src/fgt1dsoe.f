...
c    $Revision$
      ...
      subroutine fgt1d(nd,delta,eps,ns,sources,ifcharge,charge,
...
c 
      ...
      real *8 sources(ns),targ(nt)
      real *8 rnormal(ns)
      real *8 charge(nd,*),dipstr(nd,*)

      real *8 pot(nd,*),grad(nd,*),hess(nd,*)
      real *8 pottarg(nd,*),gradtarg(nd,*),hesstarg(nd,*)
...
c
      ...
      real *8 time1,time2,pi,done,pmax,bs0,cen0,bsize
...
c      
      if (npwlevel .lt. -2) then
         call fgthlterms(2,bs0,delta,eps,nlocal0)
         nlocal0 = nlocal0 + max(ifpgh,ifpghtarg) -1 
         write(6,*) ' nlocal0',nlocal0
      
         call cpu_time(time1)
C$      time1=omp_get_wtime()
         call fgt1d_large_delta(nd,delta,eps,ns,sources,
...
c     than 20 for high precision calculation
      ...
      do i = nlevstart,100
         call fgthlterms(1,bsize,delta,eps,nlocal0)
...
cccc      write(6,*) ' cutoff length', boxsize(0)/2**npwlevel/sqrt(delta)
      write(6,*) ' nperbox',ns*1.0d0/nboxes
...
c
      ...
      do i = max(npwlevel,0),nlevels
         call fgthlterms(1,boxsize(i),delta,eps,nlocal(i))
...
c     reorder sources
c
      call dreorderf(ns,sources,sourcesort,isrc)
      if(ifcharge.eq.1) 
     1    call dreorderf(nd,ns,charge,chargesort,isrc)
      if(ifdipole.eq.1) then
         call dreorderf(nd,ns,dipstr,dipstrsort,isrc)
         call dreorderf(ns,rnormal,rnormalsort,isrc)
      endif
c
cc     reorder targets
c
      call dreorderf(nt,targ,targsort,itarg)
c
c
...
c
      call g1dmpalloc(nd,itree,iaddr,
     1    nlevels,npwlevel,lmptot,
     2    ntermsh,nlocal,nsoe)
...
c
      ...
      if(ifpgh.eq.2) then
        call dreorderi(nd,ns,potsort,pot,isrc)
        call dreorderi(nd,ns,gradsort,grad,isrc)
      endif

      if(ifpgh.eq.3) then
        call dreorderi(nd,ns,potsort,pot,isrc)
        call dreorderi(nd,ns,gradsort,grad,isrc)
        call dreorderi(nd,ns,hesssort,hess,isrc)
      endif
      ...
      if(ifpghtarg.eq.2) then
        call dreorderi(nd,nt,pottargsort,pottarg,itarg)
        call dreorderi(nd,nt,gradtargsort,gradtarg,itarg)
      endif

      if(ifpghtarg.eq.3) then
        call dreorderi(nd,nt,pottargsort,pottarg,itarg)
        call dreorderi(nd,nt,gradtargsort,gradtarg,itarg)
        call dreorderi(nd,nt,hesstargsort,hesstarg,itarg)
      endif
...
c
c
      subroutine fgt1dmain(nd,delta,eps,
     $     nsource,sourcesort,
     $     ifcharge,chargesort,
     $     ifdipole,rnormalsort,dipstrsort,
     $     ntarget,targetsort,
...
c------------------------------------------------------------------
      ...
      real *8 eps,delta
      real *8 sourcesort(nsource)
      real *8 rnormalsort(nsource)
      real *8 chargesort(nd,*)
      real *8 dipstrsort(nd,*)
      real *8 targetsort(ntarget)
      real *8 pot(nd,*)
      real *8 grad(nd,*)
      real *8 hess(nd,*)
      real *8 pottarg(nd,*)
...
c
      ...
      complex *16, allocatable :: h2s(:,:)
      complex *16, allocatable :: s2l(:,:)
      complex *16, allocatable :: wsoeshift(:,:)
      complex *16, allocatable :: wsoeshiftall(:,:,:)
...
c     compute translation matrices for SOE and SOE/X expansions
     ...
     1    wsoeshift,ws,ts)
      ...
      allocate(wsoeshiftall(nsoeall,2,nmax))
      print *, 'okay so far'
...
c     Hermite to SOE
      ...
      call g1dh2smat(ntermmax,nsoe,ws,ts,h2s)

c     SOE to local
      allocate(s2l(nsoe,0:ntermmax))
      call g1ds2lmat(ntermmax,nsoe,ws,ts,s2l)
      
c     Hermite to local
      allocate(h2l(0:ntermmax,0:ntermmax))
      call g1dh2lmat(ntermmax,ntermmax,h2l)
      print *, 'okay so far'
         ...
         do ilev=npwlevel,npwlevel
            do jbox=laddr(1,ilev),laddr(2,ilev)
               ...
               if (nchild .eq. 0) then
                  ...
                  do i=1,ncoll
                     ibox = itree(iptr(7) + (jbox-1)*3+i-1)
                     nchild = itree(iptr(4)+ibox-1)
                     if (nchild .gt. 0) then
                        iflocal(jbox)=1
                        exit
...
c     compute list info
c
      call gt1d_computemnlists(nlevels,nboxes,itree,ltree,
     1    iptr,centers,
     2    boxsize,iper,mnlistsoe)
      allocate(nlistsoe(0:1,nboxes),listsoe(mnlistsoe,0:1,nboxes))
c     listsoe contains source boxes in the soe directions
      call gt1d_computelists_slow(nlevels,npwlevel,nboxes,
...
cccc           call g1dsoezero_vec(nd,rmlexp(iaddr(1,ibox)),nsoe)
cccc           call g1dsxzero_vec(nd,rmlexp(iaddr(2,ibox)),
cccc     1          npw,nsoehalf)
cccc           call g1dsoezero_vec(nd,rmlexp(iaddr(3,ibox)),nsoe)
cccc           call g1dsxzero_vec(nd,rmlexp(iaddr(4,ibox)),
cccc     1          npw,nsoehalf)
cccc           call g1dlocalzero_vec(nd,rmlexp(iaddr(5,ibox)),nlocal)
cccc           call g1dhermzero_vec(nd,rmlexp(iaddr(6,ibox)),ntermsh)
cccc
cccc         enddo
...
c                    form the Hermite expansion
                     call g1dformhc_vec(nd,delta,
     1                   sourcesort(istart),npts,
     2                   chargesort(1,istart),centers(ibox),
     3                   ntermsh(ilev),rmlexp(iaddr(4,ibox)))
c                    Hermite exp to SOE/X and SOE exp
                     call g1dh2s_vec(nd,ntermsh(ilev),
     1                   nsoe,h2s,
     2                   rmlexp(iaddr(4,ibox)),
     3                   rmlexp(iaddr(1,ibox)))
c                    Hermite exp to local exp
                     call g1dherm2local_vec(nd,nlocal(ilev),
     1                   ntermsh(ilev),h2ltmp,rmlexp(iaddr(4,ibox)),
     2                   rmlexp(iaddr(3,ibox)))
                  else
c                    form soe directly
                     call g1dformsc_vec(nd,delta,sourcesort(istart),
     1                   npts,chargesort(1,istart),centers(ibox),
     2                   nsoe,ws,ts,rmlexp(iaddr(1,ibox)))
c                    form local expansion directly
                     call g1dformlc_vec(nd,delta,sourcesort(istart),
     1                   npts,chargesort(1,istart),centers(ibox),
     2                   nlocal(ilev),rmlexp(iaddr(3,ibox)))
                  endif
               endif
            enddo
...
c                    form Hermite directly
                     call g1dformhd_vec(nd,delta,sourcesort(istart),
     1                   npts,rnormalsort(istart),
     2                   dipstrsort(1,istart),centers(ibox),
     3                   ntermsh(ilev),rmlexp(iaddr(4,ibox)))
c                    Hermite exp to SOE/X and SOE exp
                     call g1dh2s_vec(nd,ntermsh(ilev),nsoe,
     1                   h2s,rmlexp(iaddr(4,ibox)),
...
c                    form soe directly
                     call g1dformsd_vec(nd,delta,sourcesort(istart),
     1                   npts,rnormalsort(istart),
     2                   dipstrsort(1,istart),centers(ibox),
     3                   nsoe,ws,ts,rmlexp(iaddr(1,ibox)))
c                    form local directly
                     call g1dformld_vec(nd,delta,sourcesort(istart),
     1                   npts,rnormalsort(istart),
     2                   dipstrsort(1,istart),centers(ibox),
     3                   nlocal(ilev),rmlexp(iaddr(3,ibox)))
                  endif
               endif
            enddo
C$OMP END PARALLEL DO 
...
c              Check if current box is a leaf box            
               if(nchild.eq.0.and.npts.gt.0) then
                  if (ntermsh(ilev) .le. ntermswitch) then
c                    form the Hermite expansion
                     call g1dformhcd_vec(nd,delta,sourcesort(istart),
     1                   npts,chargesort(1,istart),
     2                   rnormalsort(istart),
     3                   dipstrsort(1,istart),centers(ibox),
     4                   ntermsh(ilev),rmlexp(iaddr(4,ibox)))
...
c                    Hermite exp to local expansion
     ...
     1                   rmlexp(iaddr(4,ibox)),rmlexp(iaddr(3,ibox)))                  
                  else
c                    form soe directly
                     call g1dformscd_vec(nd,delta,sourcesort(istart),
     1                   npts,chargesort(1,istart),
     2                   rnormalsort(istart),dipstrsort(1,istart),
     2                   centers(ibox),
     3                   nsoe,ws,ts,rmlexp(iaddr(1,ibox)))
c                    form local directly
                     call g1dformlcd_vec(nd,delta,sourcesort(istart),
     1                   npts,chargesort(1,istart),
     2                   rnormalsort(istart),dipstrsort(1,istart),
     3                   centers(ibox),nlocal(ilev),
...
C$OMP$SCHEDULE(DYNAMIC)
        do ibox = laddr(1,ilev),laddr(2,ilev)
          nchild = itree(iptr(4)+ibox-1)
          do i=1,nchild
            jbox = itree(iptr(5)+2*(ibox-1)+i-1)
            istart = isrcse(1,jbox)
            iend = isrcse(2,jbox)
            npts = iend-istart+1
            if(npts.gt.0) then
              dx= centers(ibox) - centers(jbox)
              ...
              call g1dshiftall_vec(nd,nsoeall,rmlexp(iaddr(1,jbox)),
...
c
            ...
            do j=1,nlistsoe(ip,ibox)
              ...
              call g1dshiftsoe_vec(nd,nsoe,rmlexp(iaddr(1,jbox)),
     1            rmlexp(iaddr(2,ibox)),wsoeshift(1,nlevels-ilev+1),ip)
            enddo
            enddo
          endif
        enddo
        ...
        do ibox = laddr(1,ilev),laddr(2,ilev)
          ...
          if(npts.gt.0) then
            do i=1,nchild
              ...
              call g1dshiftall_vec(nd,nsoeall,rmlexp(iaddr(2,ibox)),
     1            rmlexp(iaddr(2,jbox)),wsoeshiftall(1,k,nlevels-ilev))
            enddo
          endif
        enddo
C$OMP END PARALLEL DO        
...
c                        evaluate soe and sx expansions at targets
                        ...
                        if (ifpghtarg.eq.1) then
                           call g1dsevalp_vec(nd,delta,centers(ibox),
     1                         nsoe,ws,ts,rmlexp(iaddr(2,ibox)),
     2                         targetsort(istartt),
     3                         nptstarg,pottarg(1,istartt))
                        endif
                        if (ifpghtarg.eq.2) then
                           call g1dsevalg_vec(nd,delta,centers(ibox),
     1                         nsoe,ws,ts,rmlexp(iaddr(2,ibox)),
     2                         targetsort(istartt),nptstarg,
     3                         pottarg(1,istartt),gradtarg(1,istartt))
                        endif                        
                        if (ifpghtarg.eq.3) then
                           call g1dsevalh_vec(nd,delta,centers(ibox),
     1                         nsoe,ws,ts,rmlexp(iaddr(2,ibox)),
     2                         targetsort(istartt),nptstarg,
     3                         pottarg(1,istartt),gradtarg(1,istartt),
     4                         hesstarg(1,istartt))
                        endif
                        endif
c                       evaluate soe and sx expansions at sources
     ...
     1                         nsoe,ws,ts,rmlexp(iaddr(2,ibox)),
     2                         sourcesort(istarts),nptssrc,
     3                         pot(1,istarts),grad(1,istarts))
                        endif
                        if (ifpgh.eq.3) then
                           call g1dsevalh_vec(nd,delta,centers(ibox),
     1                         nsoe,ws,ts,rmlexp(iaddr(2,ibox)),
...
cccc     2                         ifcharge,chargesort,
cccc     3                         ifdipole,rnormalsort,dipstrsort,
cccc     4                         targetsort,ifpghtarg,pottarg,
cccc     5                         gradtarg,hesstarg)
cccc                        endif
cccc                        if (nptssrc .gt. 0) then
cccc                           call fgt2dpart_direct_vec(nd,delta,eps/90,
cccc     1                         istarts,iends,istarts,iends,sourcesort,
cccc     2                         ifcharge,chargesort,
cccc     3                         ifdipole,rnormalsort,dipstrsort,
cccc     4                         sourcesort,ifpgh,pot,grad,hess)
cccc                        endif
                     else
c                       convert SOE expansions to local expansions
                        call g1dsoe2local_vec(nd,nlocal(ilev),nsoe,s2l,
     1                      rmlexp(iaddr(2,ibox)),rmlexp(iaddr(3,ibox)))

c                       evaluate local expansion at targets
                        if(nptstarg.gt.0) then
                        if (ifpghtarg.eq.1) then
                           call g1dlevalp_vec(nd,delta,centers(ibox),
     1                         nlocal(ilev),rmlexp(iaddr(3,ibox)),
     2                         targetsort(istartt),
     3                         nptstarg,pottarg(1,istartt))
                        endif
                        if (ifpghtarg.eq.2) then
                           call g1dlevalg_vec(nd,delta,centers(ibox),
     1                         nlocal(ilev),rmlexp(iaddr(3,ibox)),
     2                         targetsort(istartt),nptstarg,
     3                         pottarg(1,istartt),gradtarg(1,istartt))
                        endif
                        if (ifpghtarg.eq.3) then
                           call g1dlevalh_vec(nd,delta,centers(ibox),
     1                         nlocal(ilev),rmlexp(iaddr(3,ibox)),
     2                         targetsort(istartt),nptstarg,
     3                         pottarg(1,istartt),gradtarg(1,istartt),
     4                         hesstarg(1,istartt))
...
cccc              ibox is the target box
                  ...
                  if (nptstarg .gt. 0) then
                     call fgt1dpart_direct_vec(nd,delta,eps/90,
...
c
c------------------------------------------------------------------     
      subroutine fgt1dpart_direct_vec(nd,delta,eps,istart,iend,
...
c
        ...
        if(ifcharge.eq.1.and.ifdipole.eq.0) then
          if(ifpgh.eq.1) then
             do j=jstart,jend
               call g1d_directcp_vec(nd,delta,eps,source(istart),ns,
     1            charge(1,istart),targ(j),pot(1,j))
             enddo
          endif

          if(ifpgh.eq.2) then
             do j=jstart,jend
               call g1d_directcg_vec(nd,delta,eps,source(istart),ns,
     1            charge(1,istart),targ(j),pot(1,j),grad(1,j))
             enddo
          endif
          if(ifpgh.eq.3) then
             do j=jstart,jend
               call g1d_directch_vec(nd,delta,eps,source(istart),ns,
     1            charge(1,istart),targ(j),pot(1,j),grad(1,j),
     2            hess(1,j))
             enddo
          endif
        ...
        if(ifcharge.eq.0.and.ifdipole.eq.1) then
          if(ifpgh.eq.1) then
             do j=jstart,jend
               call g1d_directdp_vec(nd,delta,eps,source(istart),ns,
     1          rnormal(istart),dipstr(1,istart),targ(j),pot(1,j))
             enddo
          endif

          if(ifpgh.eq.2) then
             do j=jstart,jend
               call g1d_directdg_vec(nd,delta,eps,source(istart),ns,
     1            rnormal(istart),dipstr(1,istart),targ(j),
     2            pot(1,j),grad(1,j))
             enddo
          endif
          if(ifpgh.eq.3) then
             do j=jstart,jend
               call g1d_directdh_vec(nd,delta,eps,source(istart),ns,
     1            rnormal(istart),dipstr(1,istart),targ(j),
     2            pot(1,j),grad(1,j),hess(1,j))
             enddo
          endif
        endif
        ...
        if(ifcharge.eq.1.and.ifdipole.eq.1) then
          if(ifpgh.eq.1) then
             do j=jstart,jend
               call g1d_directcdp_vec(nd,delta,eps,source(istart),ns,
     1            charge(1,istart),rnormal(istart),dipstr(1,istart),
     2            targ(j),pot(1,j))
             enddo
          endif
          ...
          if(ifpgh.eq.2) then
             do j=jstart,jend
               call g1d_directcdg_vec(nd,delta,eps,source(istart),ns,
     1            charge(1,istart),rnormal(istart),dipstr(1,istart),
     2            targ(j),pot(1,j),grad(1,j))
             enddo
          endif
          if(ifpgh.eq.3) then
             do j=jstart,jend
               call g1d_directcdh_vec(nd,delta,eps,source(istart),ns,
     1            charge(1,istart),rnormal(istart),dipstr(1,istart),
     2            targ(j),pot(1,j),grad(1,j),hess(1,j))
             enddo
          endif
        endif
...
c
c------------------------------------------------------------------    
      subroutine g1dmpalloc(nd,laddr,iaddr,
     1    nlevels,npwlevel,lmptot,
     2    ntermsh,nlocal,nsoe)
...
c------------------------------------------------------------------
      ...
      implicit none
      integer nlevels,npwlevel,nsoe,nd
...
c
c
      subroutine fgt1d_large_delta(nd,delta,eps,ns,sources,
     1    ifcharge,charge,ifdipole,rnormal,dipstr,iper,
     2    nlocal,center,
     3    ifpgh,pot,grad,hess,nt,targ,
     4    ifpghtarg,pottarg,gradtarg,hesstarg)
...
c 
      ...
      allocate(local(0:nlocal,nd))
c
c     form local
c
      if (ifcharge.eq.1 .and. ifdipole.eq.0) then
         call g1dformlc_vec(nd,delta,sources,ns,charge,center,
     1       nlocal,local)
      elseif (ifcharge.eq.0 .and. ifdipole.eq.1) then
         call  g1dformld_vec(nd,delta,sources,ns,rnormal,dipstr,
     1       center,nlocal,local)
      elseif (ifcharge.eq.1 .and. ifdipole.eq.1) then
         call g1dformlcd_vec(nd,delta,sources,ns,charge,rnormal,
...
c      
      if(ifpghtarg.eq.1) then
         call g1dlevalp_vec(nd,delta,center,nlocal,local,
     1       targ,nt,pottarg)
      endif
      if(ifpghtarg.eq.2) then
         call g1dlevalg_vec(nd,delta,center,nlocal,local,
     1       targ,nt,pottarg,gradtarg)
      endif
      if(ifpghtarg.eq.3) then
         call g1dlevalh_vec(nd,delta,center,nlocal,local,
