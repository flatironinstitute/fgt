...
c     when npw is an odd number, zero frequency needs special treatment
               ...
               grad(ind,k1,k2,1)=grad(ind,k1,k2,1)+dreal(cd)*2
            enddo
         enddo
c
      enddo
...
c
         do k2 = 1,n
            do k1 = 1,n
               cd = 0.0d0
               do m2 = 1,npw/2
                  cd = cd+tab_pw2deriv(m2,k2)*ff(k1,m2)
               enddo
c     when npw is an odd number, zero frequency needs special treatment
               m2=(npw+1)/2
               if (m2 .gt. npw/2) then
                  cd=cd+tab_pw2deriv(m2,k2)*ff(k1,m2)/2
               endif
               grad(ind,k1,k2,2)=grad(ind,k1,k2,2)+dreal(cd)*2
            enddo
         enddo
c
      enddo
...
c
C*********************************************************************C
      subroutine g2d_pw2pgfast(nd,n,npw,pwexp,ff,ff2,tab_pw2pot,
...
c----------------------------------------------------------------------c
      ...
      complex *16 tab_pw2deriv(npw,n)
      complex *16 ff(n,(npw+1)/2)
      complex *16 ff2(n,(npw+1)/2)
      complex *16 pwexp(npw,(npw+1)/2,nd),cd,cd2,cdx
c
      do ind = 1,nd
         do m2 = 1,(npw+1)/2
            do k1 = 1,n
               ...
               do m1 = 1,npw
                  cd = cd+tab_pw2pot(m1,k1)*pwexp(m1,m2,ind)
                  cd2 = cd2+tab_pw2deriv(m1,k1)*pwexp(m1,m2,ind)
               enddo
               ff(k1,m2) = cd
               ff2(k1,m2) = cd2
            enddo
...
c
         do k2 = 1,n
            do k1 = 1,n
               ...
               do m2 = 1,npw/2
                  cd = cd+tab_pw2pot(m2,k2)*ff(k1,m2)
                  cdx = cdx+tab_pw2pot(m2,k2)*ff2(k1,m2)
                  cd2 = cd2+tab_pw2deriv(m2,k2)*ff(k1,m2)
               enddo
c     when npw is an odd number, zero frequency needs special treatment
               m2=(npw+1)/2
               if (m2 .gt. npw/2) then
                  cd=cd+tab_pw2pot(m2,k2)*ff(k1,m2)/2
                  cdx=cdx+tab_pw2pot(m2,k2)*ff2(k1,m2)/2
                  cd2=cd2+tab_pw2deriv(m2,k2)*ff(k1,m2)/2
               endif
               pot(ind,k1,k2)=pot(ind,k1,k2)+dreal(cd)*2
               grad(ind,k1,k2,1)=grad(ind,k1,k2,1)+dreal(cdx)*2
               grad(ind,k1,k2,2)=grad(ind,k1,k2,2)+dreal(cd2)*2
            enddo
         enddo
c
      enddo
         ...
         do k2=1,n
            do k1=1,n
               ...
               do j2=1,n
                  cd=cd+taby(j2,k2)*ff(k1,j2)
               enddo
               grad(ind,k1,k2,1)=grad(ind,k1,k2,1)+cd
            enddo
         enddo
c     end of the ind loop
      enddo
...
c        transfrom in y
         do k2=1,n
            do k1=1,n
               ...
               enddo
               grad(ind,k1,k2,2)=grad(ind,k1,k2,2)+cd
            enddo
         enddo
c     end of the ind loop
      enddo
...
c
C*********************************************************************C
      subroutine leg2d_to_pglocfast(nd,n,coeff,ff,ff2,pot,grad,
     1           tabx,tabx_der,taby,taby_der)
C*********************************************************************C
c     This routine computes the volume Gauss transform over a 
c     single box source distribution given as a Legendre series.
...
c        transform in x
         do j2=1,n
            do k1=1,n
               ...
               do j1=1,n
                  cd=cd+tabx(j1,k1)*coeff(j1,j2,ind)
                  cd2=cd2+tabx_der(j1,k1)*coeff(j1,j2,ind)
               enddo
               ff(k1,j2)=cd
               ff2(k1,j2)=cd2
            enddo
         ...
         do k2=1,n
            do k1=1,n
               ...
               do j2=1,n
                  ...
                  cd2=cd2+taby_der(j2,k2)*ff(k1,j2)
               enddo
               pot(ind,k1,k2)=pot(ind,k1,k2)+cd
               grad(ind,k1,k2,1)=grad(ind,k1,k2,1)+cdx
               grad(ind,k1,k2,2)=grad(ind,k1,k2,2)+cd2
            enddo
         enddo
c     end of the ind loop
      enddo
