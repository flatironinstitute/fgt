...
c
      ...
      real *8, allocatable :: rh2x(:,:),rh2xtmp(:,:)
      real *8, allocatable :: rx2l(:,:)
      real *8, allocatable :: hexp(:,:,:,:), local(:,:,:,:)
      complex *16, allocatable :: h2x(:,:),h2xtmp(:,:)
...
c
c     Hermite to pw
cccc      allocate(h2x(npw,0:ntermmax))
      allocate(h2x(0:ntermmax,npw))
      call g3dh2xmat(ntermmax,npw,ws,ts,h2x)
cccc      allocate(rh2x(npw,0:ntermmax))
      allocate(rh2x(0:ntermmax,npw))
...
cccc      do 1100 ilev = npwlevel,npwlevel
     ...
     1       0:ntermsh(ilev),nd))
         ...
         allocate(h2xtmp(0:ntermsh(ilev),npw))
         do j1=1,npw
            do j2=0,ntermsh(ilev)
